...
- **`${#s}` is locale-dependent**, so the script picks a UTF-8 locale itself — and verifies it in a **child** process (`printf 'ä' | wc -m`), because bash keeps its old locale when `setlocale` rejects a name while still exporting the rejected value
