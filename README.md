...
`rofi-wooordhunt --modi` печатает, где этот парсер лежит. Под Nix там окажется обёртка `makeWrapper`, а не завёрнутый скрипт: голый скрипт не найдёт ни `curl`, ни `pup`.
...
tests/installer.sh        # флаги, коды выхода и настоящий install/uninstall install.sh — в песочнице
...
`nix flake check` прогоняет офлайновый набор плюс `tests/installer.sh`: упакованная обёртка разбирает настоящую страницу настоящим `curl`, каждая настройка доезжает до скрипта, модуль Home Manager вычисляется против заглушек опций
